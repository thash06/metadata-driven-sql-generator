...
STAGE_DATA_FIELDS = 'select parse_json($1):uuid as CRM_CURRENCY_ID, parse_json($1):value as ' \
                    'CURRENCY_CODE from @edw_stage_data/currency.json (file_format => json_format)'

IDENTIFIER_COL = 'CURRENCY_ID'

COLUMN_NAMES = 'CRM_CURRENCY_ID, CURRENCY_CODE'

COLUMN_NAMES_WITH_SS = 'SS.CRM_CURRENCY_ID, SS.CURRENCY_CODE'

COLUMN_NAMES_WITH_VALUE = 'CRM_CURRENCY_ID, CURRENCY_CODE'

COLUMN_NAMES_WITH_ALIAS = 'S.CRM_CURRENCY_ID, S.CURRENCY_CODE'

FOREIGN_KEY_COLS = ''

TIMESTAMP_COLUMNS = 'INSERT_UTC_DATETIME, START_DATETIME, END_DATETIME'

HASH_KEY_COLS = 'select hash(SS.CRM_CURRENCY_ID, SS.CURRENCY_CODE) as source_key'
...
def dynamic_statement_generator():
    return DynamicStatementGenerator('@edw_stage_data/currency.json', 'currency', True)
...
def test_generate_join_conditions(dynamic_statement_generator):
    ...
    verify_answer('LEFT JOIN CURRENCY T ON T.CURRENCY_ID = '
                  'S.source_key WHERE T.CURRENCY_ID is null', answer)
...
def insert_statement_generator():
    insert_cols = 'INSERT into {} (CURRENCY_ID, {}, {})'.format(TABLE_NAME, COLUMN_NAMES, TIMESTAMP_COLUMNS)
    ...
    primary_key_null_condition = 'AS S{newline}LEFT JOIN CURRENCY T ' \
                                 'ON T.CURRENCY_ID = S.source_key WHERE T.CURRENCY_ID is null'. \
