...
STAGE_DATA_FIELDS = 'select ' \
                    ...
                    'parse_json($1):contactAddress as CONTACT_ADDRESS1, ' \
                    'parse_json($1):contactCode as CONTACT_CODE, ' \
                    'parse_json($1):contactEmail as CONTACT_EMAIL, ' \
                    'parse_json($1):contactName as CONTACT_NAME, ' \
                    'parse_json($1):contactPhoneDirect as CONTACT_PHONE_DIRECT, ' \
                    'parse_json($1):contactPhoneFax as CONTACT_PHONE_FAX, ' \
                    'parse_json($1):contactRole as CONTACT_ROLE, ' \
                    'parse_json($1):contactType as CONTACT_TYPE, ' \
                    'parse_json($1):contactUUID as CRM_CONTACT_ID, ' \
                    'parse_json($1):jobTitle as CONTACT_JOB_TITLE, ' \
                    'parse_json($1):state as STATE, ' \
                    'parse_json($1):zipcode as POSTAL_CODE ' \
                    'from @edw_stage_data/contact.json (file_format => json_format)'

IDENTIFIER_COL = 'CONTACT_ID'

COLUMN_NAMES = 'CITY, CONTACT_ADDRESS1, CONTACT_CODE, CONTACT_EMAIL, ' \
               'CONTACT_JOB_TITLE, CONTACT_NAME, CONTACT_PHONE_DIRECT, ' \
               'CONTACT_PHONE_FAX, CONTACT_ROLE, CONTACT_TYPE, CRM_CONTACT_ID, POSTAL_CODE, STATE'

COLUMN_NAMES_WITH_SS = 'SS.CITY, SS.CONTACT_ADDRESS1, SS.CONTACT_CODE, SS.CONTACT_EMAIL, ' \
                       'SS.CONTACT_JOB_TITLE, SS.CONTACT_NAME, SS.CONTACT_PHONE_DIRECT, ' \
                       'SS.CONTACT_PHONE_FAX, SS.CONTACT_ROLE, SS.CONTACT_TYPE, SS.CRM_CONTACT_ID, SS.POSTAL_CODE, SS.STATE'

COLUMN_NAMES_WITH_VALUE = 'CITY, CONTACT_ADDRESS1, CONTACT_CODE, CONTACT_EMAIL, ' \
                          'CONTACT_JOB_TITLE, CONTACT_NAME, CONTACT_PHONE_DIRECT, ' \
                          'CONTACT_PHONE_FAX, CONTACT_ROLE, CONTACT_TYPE, CRM_CONTACT_ID, POSTAL_CODE, STATE'

COLUMN_NAMES_WITH_ALIAS = 'S.CITY, S.CONTACT_ADDRESS1, S.CONTACT_CODE, S.CONTACT_EMAIL, ' \
                          'S.CONTACT_JOB_TITLE, S.CONTACT_NAME, S.CONTACT_PHONE_DIRECT, ' \
                          'S.CONTACT_PHONE_FAX, S.CONTACT_ROLE, S.CONTACT_TYPE, S.CRM_CONTACT_ID, S.POSTAL_CODE, S.STATE'

FOREIGN_KEY_COLS = ''

TIMESTAMP_COLUMNS = 'INSERT_UTC_DATETIME, START_DATETIME, END_DATETIME'

HASH_KEY_COLS = 'select hash(SS.CITY, SS.CONTACT_ADDRESS1, SS.CONTACT_CODE, SS.CONTACT_EMAIL, ' \
                'SS.CONTACT_JOB_TITLE, SS.CONTACT_NAME, SS.CONTACT_PHONE_DIRECT, ' \
                'SS.CONTACT_PHONE_FAX, SS.CONTACT_ROLE, SS.CONTACT_TYPE, SS.CRM_CONTACT_ID, SS.POSTAL_CODE, SS.STATE) ' \
                'as source_key'
...
def dynamic_statement_generator():
    return DynamicStatementGenerator('@edw_stage_data/contact.json', 'contact', True)
...
def test_get_column_names(dynamic_statement_generator):
    ...
    verify_answer(', '.join(answer), '{}, {}, {}'.format(IDENTIFIER_COL, COLUMN_NAMES, TIMESTAMP_COLUMNS))
