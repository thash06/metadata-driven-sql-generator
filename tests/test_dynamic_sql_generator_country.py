...
STAGE_DATA_FIELDS = 'select parse_json($1):uuid as CRM_COUNTRY_ID, parse_json($1):value as ' \
                    'COUNTRY_NAME from @edw_stage_data/crm/country/country-2019-09-30-19:51:40.json (file_format => json_format)'

IDENTIFIER_COL = 'COUNTRY_ID'

COLUMN_NAMES = 'COUNTRY_NAME, CRM_COUNTRY_ID'

COLUMN_NAMES_WITH_VALUE = 'COUNTRY_NAME, CRM_COUNTRY_ID'

COLUMN_NAMES_WITH_SS = 'SS.COUNTRY_NAME, SS.CRM_COUNTRY_ID'

COLUMN_NAMES_WITH_ALIAS = 'S.COUNTRY_NAME, S.CRM_COUNTRY_ID'

FOREIGN_KEY_COLS = ''

TIMESTAMP_COLUMNS = 'INSERT_UTC_DATETIME, START_DATETIME, END_DATETIME'

HASH_KEY_COLS = 'select hash(SS.COUNTRY_NAME, SS.CRM_COUNTRY_ID) as source_key'
...
def dynamic_statement_generator():
    return DynamicStatementGenerator('@edw_stage_data/crm/country/country-2019-09-30-19:51:40.json', 'country', True)
...
def test_generate_join_conditions(dynamic_statement_generator):
    ...
    verify_answer('LEFT JOIN COUNTRY T ON T.COUNTRY_ID = '
                  'S.source_key WHERE T.COUNTRY_ID is null', answer)
...
def insert_statement_generator():
    insert_cols = 'INSERT into {} (COUNTRY_ID, {}, {})'.format(TABLE_NAME, COLUMN_NAMES, TIMESTAMP_COLUMNS)
    ...
    primary_key_null_condition = 'AS S{newline}LEFT JOIN COUNTRY T ' \
                                 'ON T.COUNTRY_ID = S.source_key WHERE T.COUNTRY_ID is null'. \
