...
STAGE_DATA_FIELDS = 'select ' \
                    'parse_json($1):MISSING_METADATA_FEED_ATTRIBUTE as source, ' \
                    'parse_json($1):uuid as SRC_STRATEGY_ID, ' \
                    'parse_json($1):value as STRATEGY_TYPE ' \
                    'from @edw_stage_data/crm/strategy/strategy-2019-09-30-19:51:40.json (file_format => json_format)'

IDENTIFIER_COL = 'STRATEGY_ID'

COLUMN_NAMES = 'SRC_STRATEGY_ID, STRATEGY_TYPE, source'

COLUMN_NAMES_WITH_SS = 'SS.SRC_STRATEGY_ID, SS.STRATEGY_TYPE, SS.source'

COLUMN_NAMES_WITH_VALUE = 'SRC_STRATEGY_ID, STRATEGY_TYPE, \'Satuit\''

COLUMN_NAMES_WITH_ALIAS = 'S.SRC_STRATEGY_ID, S.STRATEGY_TYPE, \'Satuit\''
...
HASH_KEY_COLS = 'select hash(SS.SRC_STRATEGY_ID, SS.STRATEGY_TYPE) as source_key'
...
def dynamic_statement_generator():
    return DynamicStatementGenerator('@edw_stage_data/crm/strategy/strategy-2019-09-30-19:51:40.json', 'strategy', True)
...
def test_generate_insert_into_statement_column_names_clause(dynamic_statement_generator):
    ...
    verify_answer(
        'INSERT into STRATEGY (' + '{}, {}, {}'.format(IDENTIFIER_COL,
                                                       COLUMN_NAMES,
                                                       TIMESTAMP_COLUMNS) + ')',
        answer)
...
def test_generate_join_conditions(dynamic_statement_generator):
    ...
    verify_answer('LEFT JOIN STRATEGY T ON T.STRATEGY_ID = '
                  'S.source_key WHERE T.STRATEGY_ID is null', answer)
...
def insert_statement_generator():
    insert_cols = 'INSERT into {} (STRATEGY_ID, {}, {})'.format(TABLE_NAME, COLUMN_NAMES, TIMESTAMP_COLUMNS)
    ...
    primary_key_null_condition = 'AS S{newline}LEFT JOIN STRATEGY T ' \
                                 'ON T.STRATEGY_ID = S.source_key WHERE T.STRATEGY_ID is null'. \
