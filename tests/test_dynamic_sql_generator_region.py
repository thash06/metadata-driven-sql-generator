...
STAGE_DATA_FIELDS = 'select parse_json($1):uuid as CRM_REGION_ID, parse_json($1):value as ' \
                    'REGION_NAME from @edw_stage_data/region.json (file_format => json_format)'

IDENTIFIER_COL = 'REGION_ID'

COLUMN_NAMES = 'CRM_REGION_ID, REGION_NAME'

COLUMN_NAMES_WITH_SS = 'SS.CRM_REGION_ID, SS.REGION_NAME'

COLUMN_NAMES_WITH_VALUE = 'CRM_REGION_ID, REGION_NAME'

COLUMN_NAMES_WITH_ALIAS = 'S.CRM_REGION_ID, S.REGION_NAME'

FOREIGN_KEY_COLS = ''

TIMESTAMP_COLUMNS = 'INSERT_UTC_DATETIME, START_DATETIME, END_DATETIME'

HASH_KEY_COLS = 'select hash(SS.CRM_REGION_ID, SS.REGION_NAME) as source_key'
...
def dynamic_statement_generator():
    return DynamicStatementGenerator('@edw_stage_data/region.json', 'region', True)
