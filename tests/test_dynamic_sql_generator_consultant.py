...
STAGE_DATA_FIELDS = 'select ' \
                    'parse_json($1):fullName as CONSULTANT_NAME, ' \
                    'parse_json($1):title as TITLE, ' \
                    'parse_json($1):uuid as CRM_CONSULTANT_ID, ' \
                    'parse_json($1):value as CONSULTANT_CODE ' \
                    'from @edw_stage_data/consultant.json (file_format => json_format)'

IDENTIFIER_COL = 'CONSULTANT_ID'

COLUMN_NAMES = 'CONSULTANT_CODE, CONSULTANT_NAME, CRM_CONSULTANT_ID, TITLE'

COLUMN_NAMES_WITH_VALUE = 'CONSULTANT_CODE, CONSULTANT_NAME, CRM_CONSULTANT_ID, TITLE'

COLUMN_NAMES_WITH_SS = 'SS.CONSULTANT_CODE, SS.CONSULTANT_NAME, SS.CRM_CONSULTANT_ID, SS.TITLE'

COLUMN_NAMES_WITH_ALIAS = 'S.CONSULTANT_CODE, S.CONSULTANT_NAME, S.CRM_CONSULTANT_ID, S.TITLE'

FOREIGN_KEY_COLS = ''

TIMESTAMP_COLUMNS = 'INSERT_UTC_DATETIME, START_DATETIME, END_DATETIME'

HASH_KEY_COLS = 'select hash(SS.CONSULTANT_CODE, SS.CONSULTANT_NAME, SS.CRM_CONSULTANT_ID, SS.TITLE) as source_key'
...
def dynamic_statement_generator():
    return DynamicStatementGenerator('@edw_stage_data/consultant.json', 'consultant', True)
